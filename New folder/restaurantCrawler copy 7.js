...
export const runCrawler = async (io, startUrl) => {
  ...
  const crawler = new CheerioCrawler({
    ...
    requestHandlerTimeoutSecs: 180, // 1 min per request
    ...
    async requestHandler({ request, $, enqueueLinks }) {
      ...
      try {
        ...
        // 💤 Pause every 250 inserts for 2 minutes
       // 💤 Pause every 250 inserted records
  if (counter % 250 === 0) {
    ...
    failedRequestHandler: async ({ request, error }) => {
      ...
      // ⚠️ Treat failedRequestHandler as error
      consecutiveErrors++;
      if (consecutiveErrors >= 10) {
        console.log("\n⚠️ 10 consecutive errors — pausing crawler for 2 minutes");
        await crawler.autoscaledPool.pause();
