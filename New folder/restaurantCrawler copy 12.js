    ...
    async requestHandler({ request, $ }) {
      ...
      try {
        ...
        if (consecutiveErrors >= 10) {
          await pauseCrawler(io, "⚠️ 10 consecutive errors — pausing for 20 minutes", 2 * 60);
          consecutiveErrors = 0;
        }
      }
    },
...
while (isRunning) {
    // Fetch a batch from DB
    const batch = await getNextRequests(BATCH_SIZE);
    if (!batch || batch.length === 0) break;

    // Map to CheerioCrawler format
    const requests = batch.map(req => ({
        url: req.url,
        userData: { id: req.id },
        uniqueKey: req.unique_key
    }));

    // Run crawler on the batch
    await crawler.run(requests);

    // Mark all handled
    for (const req of batch) {
        await dbQueue.markHandled(req.id);
    }
}
